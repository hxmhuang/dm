...
#include <petsc/finclude/petscksp.h>
    ...
    D=A*(dm_eye(n,n*2,k))
    E=dm_eye(m,n,k)*B
    F=dm_eye(m,n,k)*dm_eye(n,n*2,k) 
    X=A*2.0
    Y=2*A
    alpha=3.0
    Z=alpha*A
    U=A*alpha
    V=dm_eye(m,n,k,.false.)*dm_eye(n,n,k,.false.)
    W=3*dm_eye(m,n,k,.false.)
    if(debug) then
        if(myrank==0) print *, ">A=dm_eye(m,n,k)"
        call dm_view(A,ierr)
        if(myrank==0) print *, ">B=dm_eye(n,n*2,k)"
        call dm_view(B,ierr)
        if(myrank==0) print *, ">C=A*B"
        call dm_view(C,ierr)
        if(myrank==0) print *, ">D=A*dm_eye(n,n*2,k)"
        call dm_view(D,ierr)
        if(myrank==0) print *, ">E=dm_eye(m,n,k)*B"
        call dm_view(E,ierr)
        if(myrank==0) print *, ">F=dm_eye(m,n,k)*dm_eye(n,n*2,k)"
        call dm_view(F,ierr)
        if(myrank==0) print *, ">X=A*2.0"
        call dm_view(X,ierr)
        if(myrank==0) print *, ">Y=2*A"
        ...
        call dm_view(Y,ierr)
        if(myrank==0) print *, ">V=dm_eye(m,n,k,.false.)*dm_eye(n,n,k,.false.)"
        if(myrank==0) call dm_view(V,ierr)
    ...
    D=A .em. (dm_eye(m,n,k))
    E=dm_eye(m,n,k) .em. B
    F=dm_eye(m,n,k) .em. dm_eye(m,n,k) 
    G=A .em. A
    H=dm_eye(m,n,k,.false.) .em. dm_eye(m,n,k,.false.)
    if(debug) then
        if(myrank==0) print *, ">A=dm_eye(m,n,k)"
        call dm_view(A,ierr)
        if(myrank==0) print *, ">B=dm_eye(m,n,k)"
        ...
        call dm_view(G,ierr)
        if(myrank==0) print *, ">H=dm_eye(m,n,k,.false.) .em. dm_eye(m,n,k,.false.)"
